...
class Printer_EweiShopV2ComModel extends ComModel 
{
			...
			foreach ($print_data as $key => &$value ) 
			{
				if (($key == 'key') && !(empty($print_data['key']))) 
				{
					...
					foreach ($print_data['key'] as $ke => $val ) 
					{
						...
						else if (($type == 0) && strexists($val, '[资源价格详情]')) 
						{
							if (!(empty($params['goodsprice']))) 
							{
								$data[] = '资源小计：￥' . $params['goodsprice'];
								$data_value[] = $print_data['value'][$ke];
							}
	...
	protected function replace($params = array(), $template = '') 
	{
		...
		{
			case 1: 
				$datas = array('[收银台名称]' => (isset($params['title']) ? $params['title'] : ''), '[收银金额]' => (isset($params['price']) ? $params['price'] : ''), '[收银时间]' => (isset($params['paytime']) ? $params['paytime'] : ''), '[支付类型]' => (isset($params['paytype']) ? $params['paytype'] : ''), '[订单编号]' => (isset($params['logno']) ? $params['logno'] : ''), '[随机减金额]' => (isset($params['randommoney']) ? $params['randommoney'] : 0), '[满立减金额]' => (isset($params['enough']) ? $params['enough'] : 0), '[余额抵扣金额]' => (isset($params['deduction']) ? $params['deduction'] : 0), '[操作员]' => (isset($params['operator']) ? $params['operator'] : '管理员'), '[资源名称]' => (isset($params['goodstitle']) ? $params['goodstitle'] : ''), '[资源价格]' => (isset($params['goodsprice']) ? $params['goodsprice'] : ''), '[资源数量]' => (isset($params['goodstotal']) ? $params['goodstotal'] : ''), '[单资源合计]' => (isset($params['goodstotalprice']) ? $params['goodstotalprice'] : ''));
			break;
			default:
				$datas = array('[资源名称]' => (isset($params['shorttitle']) ? $params['shorttitle'] : ''), '[资源价格]' => (isset($params['goodsprice']) ? $params['goodsprice'] : ''), '[资源数量]' => (isset($params['goodstotal']) ? $params['goodstotal'] : ''), '[单资源合计]' => (isset($params['goodstotalprice']) ? $params['goodstotalprice'] : ''), '[资源规格]' => (isset($params['optiontitle']) ? $params['optiontitle'] : ''), '[订单编号]' => (isset($params['ordersn']) ? $params['ordersn'] : ''), '[订单金额]' => (isset($params['price']) ? $params['price'] : ''), '[优惠金额]' => (isset($params['discount']) ? $params['discount'] : ''), '[收货人]' => (isset($params['realname']) ? $params['realname'] : ''), '[收货地址]' => (isset($params['address']) ? $params['address'] : ''), '[收货电话]' => (isset($params['mobile']) ? $params['mobile'] : ''), '[备注]' => (isset($params['remark']) ? $params['remark'] : ''), '[运费]' => (isset($params['dispatchprice']) ? $params['dispatchprice'] : ''), '[订单时间标题]' => (isset($params['order_time_title']) ? $params['order_time_title'] : ''), '[订单时间]' => (isset($params['order_time']) ? $params['order_time'] : ''), '[订单状态]' => (isset($params['order_status']) ? $params['order_status'] : ''), '[门店名称]' => (isset($params['storename']) ? $params['storename'] : ''), '[门店地址]' => (isset($params['storeaddress']) ? $params['storeaddress'] : ''), '[门店联系方式]' => (isset($params['storemobile']) ? $params['storemobile'] : ''), '[门店联系人]' => (isset($params['storerealname']) ? $params['storerealname'] : ''));
			break;
			}
