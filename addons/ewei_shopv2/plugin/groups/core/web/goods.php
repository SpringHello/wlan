...
class Goods_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				plog('groups.goods.edit', '编辑拼团资源 ID: ' . $id . ' <br/>资源名称: ' . $data['title']);
			}
			else 
			{
				...
				plog('groups.goods.add', '添加拼团资源 ID: ' . $id . '  <br/>资源名称: ' . $data['title']);
			}
	...
	public function delete1() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('groups.goods.edit', '从回收站彻底删除资源<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title']);
		}
	...
	public function restore() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('groups.goods.edit', '从回收站恢复资源<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title']);
		}
		show_json(1, array('url' => referer()));
	}
	public function delete() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('groups.goods.delete', '删除拼团资源 ID: ' . $item['id'] . '  <br/>资源名称: ' . $item['title'] . ' ');
		}
		show_json(1, array('url' => referer()));
	}
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			if (!($status_update)) 
			{
				throw new Exception('资源状态修改失败！');
			}
			plog('groups.goods.edit', '修改拼团资源 ' . $item['id'] . ' <br /> 状态: ' . (($status == 0 ? '下架' : '上架')));
		}
		show_json(1, array('url' => referer()));
	}
	public function property() 
	{
		...
		if (in_array($type, array('status', 'displayorder'))) 
		{
			...
			plog('groups.goods.edit', '修改拼团资源' . $typestr . '状态   ID: ' . $id . ' ' . $statusstr . ' ');
		}
		show_json(1);
	}
}
