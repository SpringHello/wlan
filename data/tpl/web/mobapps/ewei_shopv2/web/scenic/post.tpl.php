...
<div class="page-heading">
	<span class='pull-right'>
        ...
        <a class="btn btn-primary btn-sm" href="<?php  echo webUrl('scenic/add')?>" >添加资源</a>
		<?php  } ?>
        <a class="btn btn-default btn-sm" href="<?php  echo webUrl('scenic',array('goodsfrom'=>$_GPC['goodsfrom'], 'page'=>$_GPC['page']))?>">返回列表</a>
    </span>
    <h2><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>资源 <small><?php  if(!empty($item['id'])) { ?>修改【<span class="text-info"><?php  echo $item['title'];?></span>】<?php  } ?><?php  if(!empty($merch_user['merchname'])) { ?>商户名称:【<span class="text-info"><?php  echo $merch_user['merchname'];?></span>】<?php  } ?></small></h2>
</div>
<form <?php if( ce('scenic' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
  
  <div class="form-group">
    <label class="col-sm-2 control-label must">资源名称</label>
    <?php if( ce('scenic' ,$item) ) { ?>
    <div class="col-sm-7"  style="padding-right:0;" >
        <input type="text" name="jingquname" id="jingquname" class="form-control" value="<?php  echo $item['jingquname'];?>" data-rule-required="true" />
    </div>
    <?php  } else { ?>
    <div class='form-control-static'><?php  echo $item['jingquname'];?></div>
    <?php  } ?>
</div>
		
  <div class="form-group">
    <label class="col-sm-2 control-label must">资源图片</label>
    <div class="col-sm-9 col-xs-12 gimgs">
        <?php if( ce('scenic' ,$item) ) { ?>
        <?php  echo tpl_form_field_multi_image('jingquimg',$piclist)?>
        ...
        <?php  } ?>
  		...
  		<?php if( ce('scenic' ,$item) ) { ?>
        <input type="submit" value="保存资源" class="btn btn-primary"/>
        <?php  } ?>
  </form>
	...
	window.optionchanged = false;
	 ...
	 $('form').submit(function(){
		...
		if ($("#goodsname").isEmpty()) {
			...
			$('form').attr('stop',1);
            $(this).focus(),$('form').attr('stop',1),tip.msgbox.err('请填写资源名称!');
			return false;
		}
        ...
        if(inum == 0){
            $('#myTab a[href="#tab_basic"]').tab('show');
            $('form').attr('stop',1),tip.msgbox.err('请上传资源图片!');
            return false;
        }
		...
		if (window.type == '3') {
			if (window.virtual != '0') {  //如果单规格，不能有规格
				if ($('#hasoption').get(0).checked) {
					$('form').attr('stop',1),tip.msgbox.err('您的资源类型为：虚拟物品(卡密)的单规格形式，需要关闭资源规格！');
					return false;
				}
			}
			else {
				...
				if (!has) {
					$('#myTab a[href="#tab_option"]').tab('show');
					$('form').attr('stop',1),tip.msgbox.err('您的资源类型为：虚拟物品(卡密)的多规格形式，请添加规格！');
					return false;
				}
			}
		}
			...
			if (!$('#hasoption').get(0).checked || spec_itemlen<1) {
				$('#myTab a[href="#tab_option"]').tab('show');
				$('form').attr('stop',1),tip.msgbox.err('您的资源类型为：话费流量充值，需要开启并设置资源规格！');
				return false;
			}
			if(spec_itemlen>1){
				$('#myTab a[href="#tab_option"]').tab('show');
				$('form').attr('stop',1),tip.msgbox.err('您的资源类型为：话费流量充值，只可添加一个规格！');
				return false;
			}
		}
		if (!full) {
