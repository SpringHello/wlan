...
class Index_EweiShopV2Page extends MerchWebPage 
{
	...
	public function delete() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			mplog('goods.delete', '删除资源 ID: ' . $item['id'] . ' 资源名称: ' . $item['title'] . ' ');
		}
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			mplog('goods.edit', (('修改资源状态<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['enabled']) == 1 ? '上架' : '下架'));
		}
		show_json(1, array('url' => referer()));
	}
	public function delete1() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			mplog('goods.edit', '从回收站彻底删除资源<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title']);
		}
		show_json(1, array('url' => referer()));
	}
	public function restore() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			mplog('goods.edit', '从回收站恢复资源<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title']);
		}
		show_json(1, array('url' => referer()));
	}
