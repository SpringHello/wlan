...
function user_single($user_or_uid) {
    //user = array(2) { ["username"]=> string(4) "2423" ["password"]=> string(4) "1222" }
	$user = $user_or_uid;
	if (empty($user)) {
		return false;
	}
	...
	//$sql = 'select * from ' . tablename('users') ."where username='".$user['username'] ."' limit 1";
...
function user_hash($passwordinput, $salt) {
	...
	//var_dump($_W['config']['setting']['authkey']);echo "<br>";
	$passwordinput = "{$passwordinput}-{$salt}-{$_W['config']['setting']['authkey']}";
	//var_dump(sha1($passwordinput));die;
