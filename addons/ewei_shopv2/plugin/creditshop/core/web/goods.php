...
class Goods_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				plog('creditshop.goods.edit', '编辑积分商城资源 ID: ' . $id . ' <br/>资源名称: ' . $data['title']);
			}
			else 
			{
				...
				$id = pdo_insertid();
				plog('creditshop.goods.add', '添加积分商城资源 ID: ' . $id . '  <br/>资源名称: ' . $data['title']);
			}
			if ($data['hasoption']) 
			{
				$totalstocks = 0;
	...
	public function delete() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('creditshop.goods.delete', '删除积分商城资源 ID: ' . $item['id'] . '  <br/>资源名称: ' . $item['title'] . ' ');
		}
	...
	public function property() 
	{
		...
		if (in_array($type, array('istop', 'isrecommand', 'status', 'displayorder', 'title'))) 
		{
			...
			plog('creditshop.goods.edit', '修改积分商城资源' . $typestr . '状态   ID: ' . $id . ' ' . $statusstr . ' ');
		}
		show_json(1);
	}
