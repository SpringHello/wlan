...
class Group_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($groupname)) 
			{
				show_json(0, '资源组名称不能为空');
			}
			if (empty($goodsids)) 
			{
				show_json(0, '资源组中资源不能为空');
			}
			...
			if (!(empty($item))) 
			{
				...
				plog('goods.group.edit', '修改资源组 ID: ' . $id);
			}
			else 
			{
				...
				$id = pdo_insertid();
				plog('goods.group.add', '添加资源组 ID: ' . $id);
			}
	...
	public function delete() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('goods.group.delete', '删除资源组 ID: ' . $item['id'] . ' 标题: ' . $item['name'] . ' ');
		}
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('goods.group.edit', (('修改资源组状态<br/>ID: ' . $item['id'] . '<br/>资源组名称: ' . $item['name'] . '<br/>状态: ' . $_GPC['enabled']) == 1 ? '启用' : '禁用'));
		}
		show_json(1);
	}
