...
<?php if(cv('goods')) { ?>
<div class='menu-header'>资源管理</div>
<ul class='goods-list'>
    ...
    <li <?php  if($_GPC['goodsfrom'] == 'cycle') { ?> class="active" <?php  } ?>><a href="<?php  echo webUrl('goods',array('goodsfrom'=>'cycle'))?>">回收站<span  class='text-default'><?php  echo $totals['cycle'];?></span></a></li>
     <?php if(cv('goods.category')) { ?><li <?php  if($_W['action'] == 'goods.category') { ?> class="active" <?php  } ?>><a href="<?php  echo webUrl('goods/category')?>">资源分类</a></li><?php  } ?>
    <?php if(cv('goods.group')) { ?><li <?php  if($_GPC['r'] == 'goods.group' || $_GPC['r'] == 'goods.group.edit') { ?>class="active" <?php  } ?>><a href="<?php  echo webUrl('goods/group')?>">资源组</a></li><?php  } ?>
