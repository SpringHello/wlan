...
if ($_W['ispost']) 
{
	...
	if ($diyform) 
	{
		if ($_GPC['diyformtype'] == 2) 
		{
			...
			if ($_GPC['diysave'] == 1) 
			{
				...
				$insert = array();
				$insert['title'] = '资源ID' . $item['id'] . '的自定义表单';
	...
	if (empty($id)) 
	{
		...
		$id = pdo_insertid();
		plog('goods.add', '添加资源 ID: ' . $id . '<br>' . ((!(empty($data['nocommission'])) ? '是否参与分销 -- 否' : '是否参与分销 -- 是')));
	}
	else 
	{
		unset($data['createtime']);
		pdo_update('ewei_shop_goods', $data, array('id' => $id));
		plog('goods.edit', '编辑资源 ID: ' . $id . '<br>' . ((!(empty($data['nocommission'])) ? '是否参与分销 -- 否' : '是否参与分销 -- 是')));
	}
