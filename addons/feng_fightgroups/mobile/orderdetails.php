<?php
	...
	if(!empty($id)){
		...
		$sql = 'SELECT gname,gprice,oprice,gimg,freight FROM '.tablename('tg_goods').' WHERE id=:gid and uniacid = :uniacid'; //从资源信息表里面取得数据的sql语句
		$params = array(':gid'=>$order['g_id'], ':uniacid'=>$weid);
		$good = pdo_fetch($sql, $params); //获取符合条件的第一条数据,实际上,符合条件的数据应该有且只有一条
		if(empty($good)){
			message('获取资源信息失败', $this->createMobileUrl('index'));	//如果没有找到符合条件的订单,则返回首页
		}
	}
