...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$query = 'select * from' . tablename('ajingqu').'where deleted=0';
		$arr = pdo_fetchall($query, $params);
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,jingquname FROM ' . tablename('ajingqu') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('ajingqu', array('deleted' => 1), array('id' => $item['id']));
			plog('scenic.delete', '删除资源 ID: ' . $item['id'] . ' 资源名称: ' . $item['jingquname'] . ' ');
		}
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('goods.edit', (('修改资源状态<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['status']) == 1 ? '上架' : '下架'));
		}
		show_json(1, array('url' => referer()));
	}
	public function checked() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('goods.edit', (('修改资源状态<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['checked']) == 0 ? '审核通过' : '审核中'));
		}
		show_json(1, array('url' => referer()));
	}
	public function delete1() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('goods.edit', '从回收站彻底删除资源<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title']);
		}
		show_json(1, array('url' => referer()));
	}
	public function restore() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('goods.edit', '从回收站恢复资源<br/>ID: ' . $item['id'] . '<br/>资源名称: ' . $item['title']);
		}
		show_json(1, array('url' => referer()));
	}
	...
	public function query() 
	{
		...
		if (!(empty($kwd))) 
		{
			$condition .= ' AND (`jingquname` LIKE :keywords OR `keywords` LIKE :keywords)';
			$params[':keywords'] = '%' . $kwd . '%';
		}
		if (empty($type)) 
		{
