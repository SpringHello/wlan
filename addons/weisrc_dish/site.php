...
//        include $this->template('dish_rest_list');
    ...
    public function doMobileRemoveDishNumOfCategory()
    {
        ...
        $dishid = intval($_GPC['dishid']); //资源id
        $action = $_GPC['action'];
...
//        message($result, '', 'ajax');
        ...
        foreach ($list as $key => $row) {
            ...
            $data[$key] = array(
                ...
                'total' => empty($dish_arr) ? 0 : intval($dish_arr[$row['id']]) //资源数量
            );
        }
        $result['data'] = $data;
    ...
    public function doMobileAddToOrder()
    {
        ...
        if (empty($cart)) { //购物车为空
            $this->showMessageAjax('请先添加资源!', $this->msg_status_bad);
        } else {
    ...
    public function doWebNave()
    {
        ...
        if ($operation == 'display') {
            if ($_GPC['type'] == 'default') {
                ...
                $this->insert_default_nave('资源列表', 3, '');
    ...
    public function doWebGoods()
    {
        ...
        if ($operation == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                message('资源更新成功！', $this->createWebUrl('goods', array('op' => 'display', 'storeid' => $storeid)), 'success');
            }
