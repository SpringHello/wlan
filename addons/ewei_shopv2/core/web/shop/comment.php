...
class Comment_EweiShopV2Page extends WebPage
{
	...
	public function delete()
	{
		...
		foreach ($items as $item ) {
			...
			plog('shop.comment.delete', '删除评价 ID: ' . $id . ' 资源ID: ' . $goods['id'] . ' 资源标题: ' . $goods['title']);
		}
	...
	protected function virtual()
	{
		...
		if ($_W['ispost']) {
			if (empty($goodsid)) {
				show_json(0, array('message' => '请选择要评价的资源'));
			}
			...
			if (!empty($id)) {
				...
				plog('shop.comment.edit', '编辑资源虚拟评价 ID: ' . $id . ' 资源ID: ' . $goods['id'] . ' 资源标题: ' . $goods['title']);
			}
			 else {
				pdo_insert('ewei_shop_order_comment', $data);
				$id = pdo_insertid();
				plog('shop.comment.add', '添加虚拟评价 ID: ' . $id . ' 资源ID: ' . $goods['id'] . ' 资源标题: ' . $goods['title']);
			}
	...
	public function post()
	{
		...
		if ($_W['ispost']) {
			if ($type == 0) {
				...
				pdo_update('ewei_shop_order_comment', $data, array('id' => $id));
				plog('shop.comment.post', '回复资源评价 ID: ' . $id . ' 资源ID: ' . $goods['id'] . ' 资源标题: ' . $goods['title']);
			}
			 else {
				...
				$log_msg = '资源首次评价' . $checked_array[$checked];
				...
				$log_msg .= ' ID: ' . $id . ' 资源ID: ' . $goods['id'] . ' 资源标题: ' . $goods['title'];
