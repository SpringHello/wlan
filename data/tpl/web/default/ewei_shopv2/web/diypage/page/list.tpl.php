        ...
        <div class="col-sm-7 pull-right">
            <?php  if($pagetype=='sys') { ?>
                <select name="type" class="form-control  input-sm" style="width:120px;">
                    ...
                    <option value="5" <?php  if(intval($_GPC['type'])==5) { ?> selected<?php  } ?>>资源详情页</option>
                </select>
            ...
            <tbody>
                <?php  if(is_array($list)) { foreach($list as $item) { ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?php  if($item['type']==5||$item['type']==7||$item['type']==8) { ?>javascript:tip.msgbox.err('<?php  if($item['type']==5) { ?>资源详情页<?php  } else if($item['type']==5) { ?>秒杀专题页<?php  } else { ?>兑换中心<?php  } ?>涉及资源数据问题，请至手机端预览');<?php  } else { ?><?php  echo webUrl('diypage/page/preview', array('id'=>$item['id']))?><?php  } ?>" target="_blank">
