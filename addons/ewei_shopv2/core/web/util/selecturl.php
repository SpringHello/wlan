...
class Selecturl_EweiShopV2Page extends WebPage 
{
	...
	public function query() 
	{
		...
		if (!(empty($kw)) && !(empty($type))) 
		{
			...
			{
				$list = pdo_fetchall('SELECT id,jingquname,jingquimg,jingqugonglue,opentime FROM ' . tablename('ajingqu') . ' WHERE uniacid= :uniacid and status=:status and deleted=0 AND jingquname LIKE :jingquname ', array(':jingquname' => '%' . $kw . '%', ':uniacid' => $_W['uniacid'], ':status' => '1'));
