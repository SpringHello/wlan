...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
				...
				else 
				{
					foreach ($goods as $k => $v ) 
					{
						if (!(empty($v['hasoption']))) 
						{
							...
							{
								$this->message('资源' . $v['title'] . '的规格不存在,请到购物车删除该资源重新选择规格!', '', 'error');
							}
						}
						if ($is_openmerch == 0) 
						{
			...
			else 
			{
				...
				else if ($giftid) 
				{
					...
					{
						$this->message('赠品与资源不匹配或者资源没有赠品!', '', 'error');
					}
					$giftGood = array();
				...
				{
					$followtip = ((empty($goods['followtip']) ? '如果您想要购买此资源，需要您关注我们的公众号，点击【确定】关注后再来购买吧~' : $goods['followtip']));
				...
				if (!(empty($optionid))) 
				{
					...
					{
						$this->message('资源' . $data['title'] . '的规格不存在,请重新选择规格!', '', 'error');
					}
				}
				if ($giftid) 
				{
		...
		else 
		{
			...
			{
				$this->message('套餐活动已结束，谢谢您的关注，请浏览其他套餐或资源！', '', 'error');
			}
			$goods = array();
			$goodsprice = 0;
			$marketprice = 0;
	...
	public function submit() 
	{
		...
		if (!(empty($packageid))) 
		{
			...
			if ($package['endtime'] < time()) 
			{
				show_json(0, '套餐活动已结束，谢谢您的关注，请您浏览其他套餐或资源！');
			}
		...
		foreach ($goods as $g ) 
		{
			...
			if (!(empty($data['hasoption']))) 
			{
				...
				{
					show_json(0, '资源' . $data['title'] . '的规格不存在,请到购物车删除该资源重新选择规格!');
				}
			}
			$rank = intval($_SESSION[$goodsid . '_rank']);
			$join_id = intval($_SESSION[$goodsid . '_join_id']);
		...
		{
			show_json(0, '任务活动优惠资源' . $tgoods['title'] . '不能放入购物车下单,请单独购买');
		}
		if (empty($allgoods)) 
		{
			show_json(0, '未找到任何资源');
