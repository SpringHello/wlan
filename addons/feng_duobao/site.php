...
//付款结果返回
	public function payResult($params){
		...
		if ($params['from'] == 'return') {
			$order = pdo_fetch("SELECT * FROM " . tablename('feng_record') . " WHERE id ='{$params['tid']}'");//获取资源ID
			if ($order['status'] != 1) {
				if ($params['result'] == 'success') {
					$data['status'] = 1;
					$codes = pdo_fetch("SELECT * FROM " . tablename('feng_goodscodes') . " WHERE s_id ='{$order['sid']}'");//获取资源code
					$sidm = pdo_fetch("SELECT * FROM " . tablename('feng_goodslist') . " WHERE id ='{$order['sid']}'");//获取资源详情
					$s_codes=unserialize($codes['s_codes']);//转换资源code
					...
					if ($c_number>0) {
						if ($fee<$c_number) {
							...
							//计算获奖的code和获奖人
							$s_record = pdo_fetchall("SELECT * FROM " . tablename('feng_record') . " WHERE uniacid = '{$_W['uniacid']}' and sid ='{$order['sid']}'");//获取资源所有交易记录
							...
							foreach ($s_record as $value) {
								$ss_codes=unserialize($value['s_codes']);//转换资源code
							...
							if(empty($sid_mess['q_user'])){
								$ss_codes=unserialize($data['s_codes']);//转换资源code
								for ($i=0; $i < count($ss_codes) ; $i++) { 
									if ($ss_codes[$i]==$wincode) {
