...
class Orders_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function confirm() 
	{
		...
		try 
		{
			...
			if ($goods['stock'] <= 0) 
			{
				throw new Exception('您选择的资源已经下架，请浏览其他资源或联系商家！');
			}
			$follow = m('user')->followed($openid);
			if (!(empty($goods['followneed'])) && !($follow) && is_weixin()) 
			{
				$followtext = ((empty($goods['followtext']) ? '如果您想要购买此资源，需要您关注我们的公众号，点击【确定】关注后再来购买吧~' : $goods['followtext']));
			...
			{
				throw new Exception('您已到达此资源购买上限，请浏览其他资源或联系商家！');
			}
			...
			{
				throw new Exception('该团人数已达上限，请浏览其他资源或联系商家！');
			}
			if (!(empty($teamid))) 
			{
				...
				foreach ($orders as $key => $value ) 
				{
					...
					{
						throw new Exception('该活动已过期，请浏览其他资源或联系商家！');
					}
					if ($orders && ($value['success'] == 1)) 
					{
						throw new Exception('该活动已结束，请浏览其他资源或联系商家！');
					}
				}
				...
				{
					throw new Exception('该活动已成功组团，请浏览其他资源或联系商家！');
				}
			}
			if ($type == 'groups') 
			{
