...
class Wdl_shoppingModuleSite extends WeModuleSite {
	...
	public function doWebGoods() {
		...
		if ($operation == 'post') {
			...
			if (empty($category)) {
				message('抱歉，请您先添加资源分类！', $this->createWebUrl('category', array('op' => 'post')), 'error');
			}
			if (checksubmit('submit')) {
...
//				}
				...
				message('资源更新成功！', $this->createWebUrl('goods', array('op' => 'display', 'id' => $id)), 'success');
			}
...
//			pdo_delete('shopping_goods', array('id' => $id));
	...
	//设置订单资源的库存 minus  true 减少  false 增加
...
//			$feedback['address'] = $addresses[$addressid];
	...
	public function doMobileMyCart() {
		...
		if ($op == 'add') {
			...
			if (empty($goods)) {
				$result['message'] = '抱歉，该资源不存在或是已经被删除！';
				message($result, '', 'ajax');
			}
	...
	public function setOrderCredit($orderid, $add = true) {
		...
		if (!empty($credit)) {
			...
			if (!empty($fans)) {
				if (!empty($add)) {
					mc_credit_update($_W['member']['uid'], 'credit1', $credit, array('0' => $_W['member']['uid'], '购买资源赠送'));
				} else {
	...
	public function doMobileDetail() {
		...
		if ($goods['istime'] == 1) {
			...
			if (time() > $goods['timeend']) {
				message('抱歉，资源限购时间已到，不能购买了哦~', $backUrl, "error");
			}
		}
		$title = $goods['title'];
		//浏览量
