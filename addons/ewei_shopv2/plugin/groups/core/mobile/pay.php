...
class Pay_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		{
			$this->message('该活动已失效，请浏览其他资源或联系商家！', mobileUrl('groups/index'), 'error');
		}
		...
		if (!(empty($teamid))) 
		{
			...
			foreach ($team_orders as $key => $value ) 
			{
				...
				{
					$this->message('该活动已过期，请浏览其他资源或联系商家！', mobileUrl('groups/index'), 'error');
				}
				if ($team_orders && ($value['success'] == 1)) 
				{
					$this->message('该活动已结束，请浏览其他资源或联系商家！', mobileUrl('groups/index'), 'error');
				}
			}
			...
			{
				$this->message('该活动已成功组团，请浏览其他资源或联系商家！', mobileUrl('groups/index'), 'error');
			}
		}
		if (empty($order)) 
		{
	...
	public function complete() 
	{
		...
		if (empty($order_goods)) 
		{
			...
			{
				$this->message('资源不存在!', mobileUrl('groups/orders'));
			}
		}
		$type = $_GPC['type'];
