...
//获取code码，以获取openid
		...
		$unifiedOrder->setParameter("openid", "$openid");//资源描述
		$unifiedOrder->setParameter("body", "测试贡献一分钱");//资源描述
//自定义订单号，此处仅作举例
		$timeStamp = time();
...
//$unifiedOrder->setParameter("goods_tag","XXXX");//资源标记
//$unifiedOrder->setParameter("openid","XXXX");//用户标识
//$unifiedOrder->setParameter("product_id","XXXX");//资源ID
...
//=========步骤3：使用jsapi调起支付============
	...
	public function  doMobileAjax()
	{
		...
		if ($action == 'kanjia') {//砍价
			...
			if ($leftCount <= 0) {
				echo '{"code":200,"msg":"剩余资源为0了，下次再来参与活动吧!","status":0,"dingdanid":"119201504151139359","ret":1,"action":"passgoumai"}';
			...
			if ($leftCount <= 0) {
				echo '{"code":500,"msg":"剩余资源为0了，下次再来购买吧!"}';
	...
	public function  doMobileOrderSubmit()
	{
		...
		if ($orderInfo['status'] == $this::$KJ_STATUS_XD && $wkj['pay_type'] == 1) {//立即支付
			...
			$unifiedOrder = new UnifiedOrder_pub($this->kjSetting);
			$unifiedOrder->setParameter("openid", $user['openid']);//资源描述
			$unifiedOrder->setParameter("body", "砍价资源" . $wkj['p_name']);//资源描述
