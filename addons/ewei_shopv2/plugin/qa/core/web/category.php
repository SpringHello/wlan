...
class Category_EweiShopV2Page extends PluginWebPage 
{
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('qa.category.edit', (('修改资源分类<br/>ID: ' . $item['id'] . '<br/>标题: ' . $item['name'] . '<br/>状态: ' . $_GPC['enabled']) == 1 ? '显示' : '隐藏'));
		}
	...
	public function isrecommand() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('qa.category.edit', (('修改资源分类<br/>ID: ' . $item['id'] . '<br/>标题: ' . $item['name'] . '<br/>状态: ' . $_GPC['isrecommand']) == 1 ? '推荐' : '取消推荐'));
		}
		show_json(1, array('url' => referer()));
	}
}
