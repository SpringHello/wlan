		...
		foreach ($times as $time ) 
		{
			...
			foreach ($goods as $g ) 
			{
				if (in_array($g['goodsid'], $goodsids) && ($g['roomid'] != $roomid)) 
				{
					...
					{
						$error['goods-' . $g['goodsid']] = '资源&lt;span class=\'text text-danger\'&gt;【' . $goodstitle . '】&lt;/span&gt;在会场&lt;a href=\'' . $url . '\' target=\'_blank\'&gt;【' . $room['title'] . '】&lt;/a&gt;中的 &lt;span class=\'text text-danger\'&gt;' . $time['time'] . '&lt;/span&gt; 点场已经存在，不能重复添加';
					}
				}
			}
		}
