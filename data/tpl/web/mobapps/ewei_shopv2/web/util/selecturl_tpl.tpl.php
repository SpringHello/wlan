<?php defined('IN_IA') or exit('Access Denied');?><meta charset="utf-8">
<?php  if($type=='good') { ?>
	<?php  if(empty($list)) { ?>
		<div class="tip">抱歉！未查询到<?php  if(!empty($kw)) { ?>与“<?php  echo $kw;?>”<?php  } ?>相关的资源，请更换关键字后重试。</div>
	<?php  } else { ?>
...
<?php  } else if($type=='scenic') { ?>
	<?php  if(empty($list)) { ?>
		<div class="tip">抱歉！未查询到<?php  if(!empty($kw)) { ?>与“<?php  echo $kw;?>”<?php  } ?>相关的景区信息，请更换关键字后重试。</div>
	<?php  } else { ?>
		<?php  if(is_array($list)) { foreach($list as $item) { ?>
			<div class="line">
				<div class="image"><img src="<?php  echo tomedia($item['jingquimg'])?>"/></div>
				<nav data-href="<?php  echo mobileUrl('scenic/detail', array('id'=>$item['id']), $full)?>" class="btn btn-default btn-sm" title="选择">选择</nav>
				<div class="text"><?php  echo $item['jingquname'];?></div>
			</div>
		<?php  } } ?>
	<?php  } ?>	
