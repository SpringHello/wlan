...
//解码base64
    ...
    modal.init = function (params) {
        ...
        $(".btn-save").unbind('click').click(function () {
            var status = $(this).data('status');//返回data-type的值
            var type = $(this).data('type');
            if (status) {
                tip.msgbox.err("正在保存，请稍候。。。");
                return
            }
            if (type == 'preview') {//保存并预览
                modal.save(true)
            } else if (type == 'save') {//保存页面
                modal.save()
            } else if (type = 'savetemp') {//另存为模板
                modal.initTemp();
                return
            }
        });
        ...
        var preview_id = util.cookie.get('preview_id');//获取用户预览的id
        if (preview_id) {
            setTimeout(function () {
    ...
    modal.initNavs = function () {
        ...
        //存放Navs的一些对象属性
        var navgroup = {
            0: ['listmenu', 'richtext', 'title', 'line', 'blank', 'menu', 'menu2', 'picture', 'banner', 'picturew', 'pictures', 'icongroup', 'audio', 'coupon','goupiao'],//公共的模块参数
            1: ['search', 'fixedsearch', 'notice', 'goods', 'merchgroup', 'diymod'],//自定义页面 type=1
            2: ['search', 'fixedsearch', 'notice', 'goods', 'merchgroup', 'seckillgroup', 'diymod'],//商城首页
            3: ['member', 'bindmobile', 'logout'],//会员中心type =3
            4: ['memberc', 'blockgroup'],//分销中心
            5: ['detail_tab', 'detail_swipe', 'detail_info', 'detail_sale', 'detail_spec', 'detail_package', 'detail_shop', 'detail_store', 'detail_buyshow', 'detail_comment', 'detail_pullup', 'detail_navbar', 'detail_seckill', 'goods'],//资源详情页
            6: ['goods', 'search', 'merchgroup'],//积分商城
            7: ['seckill_times', 'seckill_rooms', 'seckill_advs', 'seckill_list'],
            8: ['exchange_banner', 'exchange_input', 'exchange_rule'],//兑换中心
            99: []//打开模块管理的页面
        };
        var navpage = navgroup[modal.type];//获取模板类型，获取对应 参数
        if (navpage) {//判断参数是否
            navpage = $.merge(navpage, navgroup[0])//合并两个数组
        } else {
            navpage = navgroup[0]//选 择第一个参数
        }
        //遍历数组
        $.each(navpage, function (index, val) {
            ...
            if (params) {
                ...
                modal.initnav.push(params)//向空数组后添加 模块对象名
            }
        });
        var html = tpl("tpl_navs", modal);//返回模块列表html
        $("#navs").html(html).show();//显示 模块列表
        $("#navs nav").unbind('click').click(function () {//点击模块名
            var id = $(this).data('id');//定义id为模块对象名
            if (id === 'page') {
                $("#page").trigger("click");//触发$("#page")的点击事件
                return
            }
            if (id == 'merchgroup' && !modal.plugins.merch) {//如果是商户组且存在merch属性
                tip.msgbox.err("禁止添加此元素！");//提示错误
                return
            }
            ...
            var inArray = $.inArray(id, navpage);//模块的序列号
            if (inArray < 0) {
                tip.msgbox.err("此页面类型禁止添加此元素！");
                return
            }
            var item = $.extend(true, {}, modal.navs[id]);//modal.navs[id]模块对象
            delete item.name;//删除name属性
            if (!item) {//如果Item没有对象
                tip.msgbox.err("未找到此元素！");
                return
            }
            var itemTplShow = $("#tpl_show_" + id).length;//模块<script></script>id标签的长度
            var itemTplEdit = $("#tpl_edit_" + id).length;//模块<script></script>id标签长度
            	
            if (itemTplShow == 0 || itemTplEdit == 0) {
                tip.msgbox.err("添加失败！模板错误，请刷新页面重试");
                return
            }
            if (id === 'diymod') {//如果点击的id是公共模块
                modal.initMod(item);
                return
            }
            var itemid = modal.getId("M", 0);//时间戳
            if (item.data) {//判断是否存在资源信息
                var itemData = $.extend(true, {}, item.data);//递归合并
                var newData = {};
                var index = 0;
                ...
                item.data = newData//修改资源数据信息
            }
            if (item.max && item.max > 0) {//判断存在 只允许添加一个
            ...
            var append = true;
            if (modal.selected && modal.selected != 'page') {//除了第一个添加的模块
                var thisitem = modal.items[modal.selected];//获取当前点击模块的对象
                //辅助线
            ...
            } else if (modal.selected && modal.selected != 'page' && append) {//第二个添加以后
                var newItems = {};
                $.each(modal.items, function (id, eachitem) {
                    newItems[id] = eachitem;
    ...
    modal.getId = function (S, N) {//返回指定格式的时间戳
        var date = +new Date();
        var id = S + (date + N);
        return id
    };

    modal.getNavs = function () {//模块默认配置
        modal.navs = {
        		goupiao: {
                    name: '购票',
                    params: {hidetext: 0, showtype: 0, rownum: 1, showbtn: 0},
                    style: {
                        background: "#ffffff",
                        paddingtop: "3",
                        paddingleft: "5",
                    ...
                    data: {
                    	C0123456789101: {
                            imgurl: 'images/4/2017/09/B37YRuc33r3uo8Wh1b3ybHHc78aw8C.jpg',
                            linkurl: '',
                            title: '这里是上标题',
                            text: '这里是下标题'
                        },
            ...
            detail_tab: {
                ...
                type: 5,
                max: 1,
                params: {goodstext: "资源", detailtext: "详情"},
            ...
            detail_sale: {
                ...
                data: {
                    ...
                    C0123456789106: {name: "资源标签", type: "biaoqian"}
                }
            },
            detail_spec: {
                name: "资源规格",
    ...
    modal.initItems = function (selected) {
        var phone = $("#phone");//获取手机内显示部分
        if (!modal.items){//如果未添加模块时 定义modal.items为空对象
            modal.items = {};
            return
        }
        phone.empty();//清空模块
        $.each(modal.items, function (itemid, item) {
            if (typeof(item.id) !== 'undefined') {//模块执行以下代码
                var newItem = $.extend(true, {}, item);
                newItem.itemid = itemid;//给对象 添加属性 时间戳
        ...
        var btnhtml = $("#edit-del").html();//获取编辑和删除
        $("#phone .drag").append(btnhtml);//向每个模块添加编辑和删除
        $("#phone .drag .btn-edit-del .btn-del").unbind('click').click(function (e) {//点击删 除模块
            e.stopPropagation();//禁止事件冒泡
            var drag = $(this).closest(".drag");//找到整个模块
            var itemid = drag.data('itemid');//获取当前data-itemid属性值
            var nodelete = $(this).closest(".drag").hasClass("nodelete");//判断是否存在nodelete的class名
            if (nodelete) {
                tip.alert("此元素禁止删除");
                return
            }
            tip.confirm("确定删除吗", function () {
                ...
                delete modal.items[itemid];//删除属性值
                modal.initItems();
                if (nearid) {
    ...
    modal.initPage = function (initE) {//打开页面自动加载
        ...
        $("#page").text(modal.page.title);//修改标题
    ...
    modal.initSortable = function () {
        ...
        $(document).on('mousedown', "#phone .drag", function () {//选择当前模块时 给其加上selected其他删除selected
    ...
    modal.initEditor = function (scroll) {//编辑功能
        if (typeof(scroll) === 'undefined') {
            scroll = true
        }
        var itemid = modal.selected;
        var top = 180;
        //初始加载页面时modal.selected为page
        if (modal.selected != 'page') {
        ...
        if (modal.selected) {
            if (modal.selected == 'page') {//如果初始加载页面
                if (modal.type == 99) {
        ...
        if (sliderlength > 0) {
            $("#diy-editor .slider").each(function () {
                var decimal = $(this).data('decimal');//10
        ...
        var goodsSelector = $("#diy-editor .goods-selector").length;//选中的是公告时长度为4其他 长度为零
        if (goodsSelector > 0) {
        ...
        $("#diy-editor").find(".diy-bind").bind('input propertychange change', function () {//propertychange 当input的value发生改变时触发，
            var _this = $(this);//表单
    ...
    modal.initSortableChild = function () {
        $("#diy-editor .inner").sortable({//调用拖拽插件
            opacity: 0.8,
            placeholder: "highlight",
            items: '.item',
            revert: 100,
    ...
    modal.initTemp = function () {
        ...
        $("#saveTempModal").modal();//执行弹出层
        $("#saveTemp", "#saveTempModal").unbind('click').click(function () {//点击保存和另存为模板
            var tempname = $.trim($("#saveTempModal").find("#saveTempName").val());//未命 名模板
            var tempcate = $.trim($("#saveTempModal").find("#saveTempCate option:selected").val());
            var temppreview = $.trim($("#saveTempModal").find("#saveTempPreview").val());//trim删除字符串第一个和最后一个的空格
            var tempdata = {page: modal.page, items: modal.items};//将page,items存在tempdata对象里 
            if (!tempname) {
    ...
    modal.save = function (preview) {
        ...
        if (preview && modal.type == 5) {
            tip.msgbox.err("资源详情页涉及资源数据问题，请至手机端预览");
            return
        }
        if (modal.keyworderr) {
