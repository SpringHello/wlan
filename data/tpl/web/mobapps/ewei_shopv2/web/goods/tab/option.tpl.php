...
<div class="form-group">
    ...
    <div class="col-sm-11">
        ...
        <span class="help-block">资源的剩余数量, 如启用多规格<?php  if(com('virtual')) { ?>或为虚拟卡密产品<?php  } ?>，则此处设置无效.</span>
        <?php  } else { ?>
...
<div class="form-group">
       ...
       <?php if( ce('goods' ,$item) ) { ?>
	<label class="checkbox-inline">
		<input type="checkbox" id="hasoption" value="1" name="hasoption" <?php  if($item['hasoption']==1) { ?>checked<?php  } ?> />启用资源规格
	</label>
	  <span class="help-block">启用资源规格后，资源的价格及库存以资源规格为准,库存设置为0则会到”已售罄“中，手机也不会显示, -1为不限制</span>
...
<script language="javascript">
	...
	function addSpec(){
                    ...
                    if(type==3 && virtual==0 && len>=1){
						tip.msgbox.err('您的资源类型为：虚拟物品(卡密)的多规格形式，只能添加一种规格！');
                        return;
                    }
					if(type==10 && len>=1){
						tip.msgbox.err('您的资源类型为：话费流量充值，只能添加一种规格！')
						return;
					}
