...
class Goods_EweiShopV2Page extends PluginWebPage 
{
	...
	public function add() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			{
				$data = array('title' => $title, 'endtime' => $endtime, 'type' => $type, 'max' => $max, 'mode' => 1, 'status' => 1, 'uniacid' => $_W['uniacid'], 'status' => 1, 'value' => $value, 'starttime' => $starttime, 'goods' => $json_goods, 'img' => '../addons/ewei_shopv2/plugin/exchange/static/img/exchange.jpg', 'title_reply' => '资源兑换', 'content' => '欢迎来到兑换中心,点击进入兑换', 'code_type' => intval($_GPC['code_type']), 'binding' => $binding, 'postage' => $postage, 'postage_type' => $postagetype, 'showcount' => $showcount);
	...
	public function creat() 
	{
		...
		if ((0 < intval($_GPC['ajax'])) && (0 < intval($_GPC['loop']))) 
		{
			...
			while (0 < $num) 
			{
				...
				$qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['uniacid'], 'type' => 'scene', 'extra' => 0, 'qrcid' => $scene, 'name' => '资源兑换', 'keyword' => md5($key), 'model' => 1, 'ticket' => $ticket['ticket'], 'url' => $ticket['url'], 'expire' => $ticket['expire_seconds'], 'subnum' => 0, 'createtime' => time(), 'status' => 1, 'scene_str' => '');
