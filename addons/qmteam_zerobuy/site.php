...
class Qmteam_zerobuyModuleSite extends WeModuleSite {
	...
	public function doWebList() {
		...
		if($act == 'delete'){
			...
			message('删除资源成功！', $this->createWebUrl('list', array('op' => 'display')), 'success');
		}
		if($_GPC['submit']){
			$data = array();
			...
			if($act == 'add'){
				...
				message('添加资源成功！', $this->createWebUrl('list', array('op' => 'display')), 'success');
			}else{
				pdo_update('zerobuy_list', $data, array('id'=>$_GPC['id']));
				message('更新资源成功！', $this->createWebUrl('list', array('op' => 'display')), 'success');
			}
		}
	...
	public function make_code($did){
		...
		$message = '<p>资源名：'.$info['goods_title'].'</p>';
